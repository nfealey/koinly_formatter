        ...
        try:
            if self.invoices_path is None:
                if os.path.exists('invoices.csv'):
                    ...
                    raise FileNotFoundError(
                        "invoices.csv not found and no path provided"
                    )
            else:
            ...
            if self.payments_path is None:
                if os.path.exists('payments.csv'):
                    ...
                    raise FileNotFoundError(
                        "payments.csv not found and no path provided"
                    )
            else:
            ...
            if self.onchain_path is None:
                if os.path.exists('onchain.csv'):
                    ...
                    raise FileNotFoundError(
                        "onchain.csv not found and no path provided"
                    )
            else:
            ...
            for file_type, file_path in known_paths.items():
                ...
                if not os.path.exists(file_path):
                    raise FileNotFoundError(
                        f"{file_type} file not found at path: {file_path}"
                    )
                ...
                if not file_path.lower().endswith('.csv'):
                    raise ValueError(
                        f"{file_type} file must be a CSV file: {file_path}"
                    )
            ...
            for csv_type, known_path in known_paths.items():
                try:
                    with open(known_path, mode='r', encoding='utf-8') as file:
                        ...
                        for row_num, line in enumerate(csv_file, start=2):
                            ...
                            for key, value in line.items():
                                ...
                                if key in ignore_keys:
                                    ...
                                    try:
                                        ...
                                        raise ValueError(
                                            f"Invalid amount value in {csv_type} "
                                            f"row {row_num}: '{value}'"
                                        )
                                    ...
                                    try:
                                        ...
                                        raise ValueError(
                                            f"Invalid fee value in {csv_type} "
                                            f"row {row_num}: '{value}'"
                                        )
                                elif key not in convert_keys:
                                    # Log warning but continue processing
                                    logger.debug(
                                        f"Unknown field '{key}' in {csv_type} - skipping"
                                    )
                                    continue
                                else:
                    ...
                    raise ValueError(
                        f"Invalid CSV format in {csv_type} file at line {row_num}: {e}"
                    )
